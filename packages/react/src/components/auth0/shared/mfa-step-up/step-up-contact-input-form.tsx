...
interface StepUpContactInputFormProps {
  factorType: MFAType;
  enrollMfa: (
    factorType: MFAType,
...
export function StepUpContactInputForm({
  ...
  const handleCancel = () => {
    form.reset();
    setContactData({ contact: '', authSession: '', authenticationMethodId: '' });
    onClose?.();
  };
    ...
    <div style={currentStyles?.variables} className="w-full max-w-sm mx-auto">
      <div className="flex flex-col items-center justify-center flex-1 space-y-10">
        {loading ? (
          ...
          <>
            <p
              className={cn('text-center text-primary text-sm font-normal')}
                ...
                >
                  ...
                  <div className="flex flex-row justify-end gap-3 mt-6 mb-6">
                    ...
                    <Button
                      ...
                      aria-label={t('submit')}
                    >
                      {loading ? <Spinner size="sm" colorScheme="foreground" /> : t('submit')}
                    </Button>
                  </div>
                </form>
              </Form>
