...
describe('SsoProviderTableView', () => {
  const mockProps = createMockSsoProviderTableViewProps();

  it('renders the table and header', () => {
    renderWithProviders(<SsoProviderTableView {...mockProps} />);
    expect(screen.getByRole('table')).toBeInTheDocument();
    expect(screen.getByRole('banner')).toBeInTheDocument();
  });

  it('renders empty state when data is empty', () => {
    renderWithProviders(<SsoProviderTableView {...mockProps} providers={[]} />);
    expect(screen.getByText(/empty/i)).toBeInTheDocument();
  });

  it('disables create button when readOnly is true', () => {
    renderWithProviders(<SsoProviderTableView {...mockProps} readOnly={true} />);
    const createButton = screen.getByRole('button', { name: /create/i });
    expect(createButton).toBeDisabled();
  });

  it('renders loading state when isLoading is true', () => {
    renderWithProviders(<SsoProviderTableView {...mockProps} isLoading={true} />);
    expect(screen.getByRole('table')).toBeInTheDocument();
  ...
  it('renders custom header class if provided', () => {
    renderWithProviders(
      <SsoProviderTableView
        ...
        styling={{
          ...mockProps.styling,
          classes: { ...mockProps?.styling?.classes, 'SsoProviderTable-header': 'custom-header' },
        }}
      />,
    );
  ...
  it('renders custom table class if provided', () => {
    renderWithProviders(
      <SsoProviderTableView
        ...
        styling={{
          ...mockProps.styling,
          classes: { ...mockProps?.styling?.classes, 'SsoProviderTable-table': 'custom-table' },
        }}
      />,
    );
