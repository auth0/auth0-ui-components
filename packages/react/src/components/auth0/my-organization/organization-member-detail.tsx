...
/**
 * Props for the OrganizationMemberDetailView component.
 */
...
/**
 * Details tab content — user details card + danger zone.
 * @param root0 - Component props containing state and handlers
 * @returns The rendered details tab element, or null if no member is loaded
 */
function OrganizationMemberEditDetailsTab({
  state,
  handlers,
  ...
  return (
    <div className="flex flex-col gap-4">
      <MemberDetailUserDetails member={state.member} customMessages={state.customMessages} />
      <MemberDetailDangerZone
        isRemovingFromOrg={state.isRemovingFromOrg}
        isDeletingMember={state.isDeletingMember}
        customMessages={state.customMessages}
        onRemoveFromOrgClick={handlers.handleRemoveFromOrgClick}
        onDeleteMemberClick={handlers.handleDeleteMemberClick}
      />
    </div>
  );
}
...
/**
 * Roles tab content — roles table with assign/remove.
 * @param root0 - Component props containing state and handlers
 * @returns The rendered roles tab element
 */
function OrganizationMemberEditRolesTab({
  state,
  handlers,
